...
namespace DeadMosquito.AndroidGoodies.Internal
{
	...
	public static class AndroidDialogUtils
	{
		...
		{
			var window = dialog.CallAJO("getWindow");
			const int FLAG_NOT_FOCUSABLE = 8;
			window.Call("setFlags", FLAG_NOT_FOCUSABLE, FLAG_NOT_FOCUSABLE);
			dialog.Call("show");
			var currentVisibility = AGUtils.Window.CallInt("getSystemUiVisibility");
			window.CallAJO("getDecorView").Call("setSystemUiVisibility", currentVisibility);
			window.Call("clearFlags", FLAG_NOT_FOCUSABLE);
		}
	}
}
#endif